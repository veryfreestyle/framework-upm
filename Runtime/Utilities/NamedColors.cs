...
namespace VeryFS.Framework.Runtime.Utilities
{
    public static class NamedColors
    {
        ...
        public static Color ToColor(this JsonData data)
        {
            string a = (string)data;
           
            if (ColorUtility.TryParseHtmlString(a, out var color))
            {
                return color;
            }
            return Color.black;
        ...
        public static JsonData ToJsonData(this Color color)
        {
            string str = "#" + ColorUtility.ToHtmlStringRGBA(color);
            var data = new JsonData(str);
            return data;
        }
    }
}