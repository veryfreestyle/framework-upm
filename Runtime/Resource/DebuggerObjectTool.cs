...
namespace VeryFS.Framework.Runtime.Resource
{
    ...
    public static class DebuggerObjectTool
    {
        ...
        public static void SetParent(string bigType, string smallType, GameObject obj)
        {
            ...
            obj.AsChild(theParent.gameObject);
            
            // try
            // {
            //     Tools.SetChild(obj, theParent.gameObject);
            // }
            // catch (Exception e)
            // {
        ...
        {
            // if (!ResourceLoaderDebugger.IsApplicationQuit)
            // {
                if (obj != null)
        ...
        private static string GetNameWithCount(string smallType, int count)
        {
            return $"{smallType}({count})";
        }

        private static Transform GetParent(string bigType, string smallType)
        {
            var uri = GetUri(bigType, smallType);
            Transform theParent;
            ...
            {
                var bigTypeObjName = $"__{bigType}__";
                ...
                theParent.AsChild(bigTypeObj.transform);
